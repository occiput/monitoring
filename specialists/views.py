...
def estim_edit(request, spec_id):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            if DictObj.objects.get(criteria=estim.criteria):
                estims = DictObj.objects.filter(key=estim.criteria, container=get_object_or_404(Dict, id=spec_id))
                estims.delete()
...
def spec_delete(request, spec_id):
    try:
        spec = Specialist.objects.get(id=spec_id)
        spec.delete()
        return HttpResponseRedirect("/")
    except Specialist.DoesNotExist:
        return HttpResponseNotFound("<h2>Specialist not found</h2>")
